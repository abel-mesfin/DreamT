...
<link rel="icon" href="./images/favicon.png" type="image/png" sizes="16x16">
    ...
    <link rel="stylesheet" href="./css/editModules.css">  
...
<body>
    <div class="topnav">
        ...
        <div class="btnright">
            ...
            <div class="dropdown">
                ...
                <a href="editModules.php">All Modules</a>
    ...
    <div class="page">
        ...
        echo "<div class='homepage' > <h1>Edit Modules</h1></div>";

        echo '
        <form method="post">
        <label class="tagLabel">Search Modules: </label>';
        ...
        if(isset($_POST["submit"])){
           ...
           //print_r($finalAllPosts[1][0]);
            ...
            for($x=0;$x < count($userPosts);$x++){
                ...
                echo '<label class="checkLabel" >Publish: </label>';
                echo '<input type="checkbox" onclick="updatePosts('.$userPosts[$x]['ID'].', '.$user_ID.' )" class="chekbox" name="publishedM'.$userPosts[$x]['ID'].'" id="publishedM'.$userPosts[$x]['ID'].'" '.$checked.'>';
            ...
            foreach($str_arr as $ke => $ne) {
                //$ar = getAllPostsByID($ne)[0];
                //array_push($arr, $ar);
                //check box based on pusblishedMX
                //echo $ne;
                ...
                function func() {
                    try{
                    var de = document.getElementById('publishedM".$ne."');
                    de.checked = true;
                    console.log(de);
...
function updatePosts(d,e) {
    let formData = new FormData();

    formData.append("posts", d);
    formData.append("user", e);
      fetch('/sqlCommand.php', {
        method: 'POST',
        body: formData,
      }).then((response) => {
    ...
    <?php 
    //"UPDATE Organization SET posts='".$."',  image_name='".$quiz."' WHERE ID =".$postID;
